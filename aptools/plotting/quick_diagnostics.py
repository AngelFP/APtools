...
def phase_space_overview(x, y, z, px, py, pz, q):
    ...
    plt.text(0.3, 0.9, '$\\epsilon_{n,x} = $' + '{}'.format(np.around(em_x, 3))
             + '$\\ \\mathrm{\\mu m \\ rad}$', transform=ax_1.transAxes,
             fontsize=8)
    ax_2 = plt.subplot(132)
    plt.plot(y * 1e6, py, '.', ms=1)    
    plt.xlabel("y [$\\mu m$]")
    plt.ylabel("$p_y \\ \\mathrm{[m_ec^2/e]}$")
    plt.text(0.3, 0.9, '$\\epsilon_{n,y} = $' + '{}'.format(np.around(em_y, 3))
             + '$\\ \\mathrm{\\mu m \\ rad}$', transform=ax_2.transAxes,
             fontsize=8)
